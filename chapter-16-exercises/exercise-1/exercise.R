...
?sample_n
diamonds_sample <- sample.n(diamonds, 100)
...
ggplot(data = midwest, mapping = aes(x = percollege, y = percadultpoverty)) +
  geom_point()+
  geom_smooth()+
  geom_point(mapping = aes(y= percchildbelowpovert),color = "red")



ggplot(data = midwest) +  geom_point(mapping = aes(x = percollege, y = peradultpoverty))

scale_x_reverse()+
  scale_y_continuous()+
  scale_color_he(l=70, c= 30)
